function [Y,X,I,J,C,imref,immarker,imorg,im00]=locate(imorg,p,clevel)
%function [X,Y,I,J]=locate(im,p,clevel)
...
if 0
%extract initial marker
imref=im(p0(1)+[1:2*sx],p0(2)+[1:2*sy]);
else
%use gaussian spot
 ...
 for kt=1:8*kr
   ...
   if ((x>0) & (x<=nx)) & ((y>0) & (y<=ny));
     ...
     if c12>clevel
       ...
       x=x+mx-1-sx;
       y=y+my-1-sy;
       % is this point still in the initial image?
       if (x>0) & (x<=nx) & (y>0) & (y<=ny);
        X=[X;x];
        Y=[Y;y];
        I=[I;m(1)];
