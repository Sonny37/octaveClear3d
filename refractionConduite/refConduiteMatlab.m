...
for k=1:iterations+1;
	...
	if(delta >= 0) % 2 intersection ou tangente %dans tus les cas on determine le 1er point atteint
		...
		if(delta1 >= 0) % 2 intersection ou tangente %dans tous les cas on determine le 1er point atteint
			...
			%if( y3<=R2 ) %si le point d'intersection est dans le cercle interne
				...
				make_latex;
				hrk1(k)=y3;
				%['i', num2str(iterations) ,'/hrk12_', num2str(k), '.png']
				if((cas2 == 0) && (exist(['i', num2str(iterations) ,'/hrk12_', num2str(k)])== 0))
					
					print(1,['i', num2str(iterations) ,'/hrk12_', num2str(k), ], "-png", "-S1400,750", "-color", "-solid")
	...
	if(delta1 >=0) %au moins une intersection avec le coté du carré
		...
		%if(y3<=R2) %si le point d'intersection est dans le cercle
		 ...
		 xlabel("x [m]");ylabel("y [m]");
		 title(sprintf("R%cfraction %c travers une interface plane Air-PMMA d'ar%cte %d mm suivie d'une interface circulaire PMMA-Eau de diam%ctre %d mm",char(233),char(224),char(232),2000*D,char(232),2000*R2), "fontsize", 15)
		 make_latex;
		 hrk2(k)=y3;
		 if(exist(['i', num2str(iterations) ,'/hrk12_', num2str(k), '.png'], 'file') == 0)
		    print(1,['i', num2str(iterations) ,'/hrk12_', num2str(k)], "-dpng", "-S1400,750", "-color", "-solid")
			endif
        %else
